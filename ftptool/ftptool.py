'''
	...
	This class is implemented top of ftplib library.
	You can also advance this library.
	
	Install Syntax: 
		Sudo python Setup.py install

	I prefer, before you install this package, you to install python dev package.
...
class FTPHost(object):
    ...
    def sync_to_remote(self, source, destination, create_destination=False,
            ignore_dotfiles=True):
        """Upload local directory `source` to remote destination `destination`.
		...
		local_source_file = path.join(current_dir, filename)
		remote_dest_file = posixpath.join(remote_dest_dir, filename)
        	localfilesize =  os.path.getsize(local_source_file)
		try:
			if self.ftp_obj.size(remote_dest_file) == localfilesize:
				print 'File already exists in ' + remote_dest_file
            		else:
				print 'File is modified, ' + remote_dest_file
                
		except ftplib.Error:
			pass
			print 'Uploaded file ' + local_source_file + ', size is ' + str(localfilesize)
			try:
				f = self.file_proxy(remote_dest_file)
				f.upload_from_file(local_source_file)
			except ftplib.Error:
